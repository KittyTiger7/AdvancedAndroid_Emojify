...
class Emojifier {
    ...
    private static void whichEmoji(Face face){
        ...
        final Double SMILING_PROB_THRESHOLD = 0.5;
        final Double EYE_OPEN_PROB_THRESHOLD = 0.5;
        ...
        boolean smilingBoo = face.getIsSmilingProbability() >= SMILING_PROB_THRESHOLD;
        boolean leftEyeOpenBoo = face.getIsLeftEyeOpenProbability() >= EYE_OPEN_PROB_THRESHOLD;
        boolean rightEyeOpenBoo = face.getIsRightEyeOpenProbability() >= EYE_OPEN_PROB_THRESHOLD;

        // TODO (5): Create an if/else system that selects the appropriate emoji based on the above booleans and log the result.
        if (smilingBoo) {
            if (leftEyeOpenBoo && rightEyeOpenBoo) Log.d("whichEmoji() chose", "smile" );
            if (leftEyeOpenBoo && !rightEyeOpenBoo) Log.d("whichEmoji() chose", "right wink" );
            if (!leftEyeOpenBoo && rightEyeOpenBoo) Log.d("whichEmoji() chose", "left wink" );
            if (!leftEyeOpenBoo && !rightEyeOpenBoo) Log.d("whichEmoji() chose", "closed eyed smile" );
        } else {
            if (leftEyeOpenBoo && rightEyeOpenBoo) Log.d("whichEmoji() chose", "frown" );
            if (leftEyeOpenBoo && !rightEyeOpenBoo) Log.d("whichEmoji() chose", "right wink frown" );
            if (!leftEyeOpenBoo && rightEyeOpenBoo) Log.d("whichEmoji() chose", "left wink frown" );
            if (!leftEyeOpenBoo && !rightEyeOpenBoo) Log.d("whichEmoji() chose", "closed eyed frown" );
        }
    }
